...
# LOAD DATA
df = pd.read_csv("data/GMStock.csv", parse_dates=["date"])
...
# PLOT GM STOCK PRICE OVER TIME
plt.figure(num="GM Stock Price (1990–2024)", figsize=(12,6))
plt.plot(df["date"], df["PRC"])
plt.title("GM Stock Price (1990–2024)")
plt.xlabel("Date")
plt.ylabel("Price ($)")
plt.show()

# PLOT TRADING VOLUME OVER TIME
plt.figure(num="GM Trading Volume (1990–2024)", figsize=(12,4))
plt.plot(df["date"], df["VOL"])
plt.title("GM Trading Volume (1990–2024)")
plt.xlabel("Date")
plt.ylabel("Volume")
plt.show()
...
plt.figure(num="GM Cumulative Return (1990–2024)", figsize=(12,6))
plt.plot(df["date"], df["cum_return"])
plt.title("GM Cumulative Return (1990–2024)")
plt.xlabel("Date")
