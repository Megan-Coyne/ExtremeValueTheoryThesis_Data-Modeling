...
# Shift all returns so the minimum value is > 0
shift = abs(df["RET"].min()) + 1  # add 1 to avoid log(0)
df["RET_log"] = np.log(df["RET"] + shift)

# Resample monthly and take maxima of the log-transformed data
block_maxima = df["RET_log"].resample("ME").max().dropna()

# Fit GEV distribution
shape, loc, scale = gev.fit(block_maxima)
print(f"[Block Maxima - GEV, Log-Transformed] shape={shape:.4f}, loc={loc:.4f}, scale={scale:.4f}")
...
plt.hist(block_maxima, bins=50, density=True, alpha=0.6, color='skyblue', label="Monthly Max Log Returns")
plt.plot(x, pdf, 'r-', lw=2, label="GEV Fit")
plt.title("Ford GEV Fit to Block Maxima of Log-Transformed Monthly Returns")
plt.xlabel("Log of Monthly Maximum Return")
plt.ylabel("Density")
plt.legend()
plt.show()
